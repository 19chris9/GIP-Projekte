...
{
	for (int i = 0; i < box_max; i++) {
		boxes[i].y = boxes[i].y + boxes[i].delta_y;
...
void handle_mouse_click(Box boxes[])
{
	...
	for (int i = 0; i < box_max; i++) {
		if (mouse_x >= boxes[i].x && mouse_x <= boxes[i].x + 40 && mouse_y >= boxes[i].y && mouse_y <= boxes[i].y + 40) {
			boxes[i].delta_y = boxes[i].delta_y + 10;
			boxes[i].y = boxes[i].y = 0;
		}
	}
}
int main()
